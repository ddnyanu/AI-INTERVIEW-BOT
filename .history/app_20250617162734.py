...
def interview(token):
    try:
        ...
        logger.debug(f"🔍 Requesting interview data from: {DJANGO_API_URL}{token}/")
        logger.debug("🌐 Interview response status: %s", interview_response.status_code)
        logger.debug("Response content: %s", interview_response.text)
        ...
        if interview_response.status_code == 200:
            ...
            resume_jd_url = f"https://ibot-backend.onrender.com/jobs/resume-jd-by-id/{match_id}/"
            resume_jd_response = requests.get(resume_jd_url, timeout=30)
            logger.debug(f"🔍 Requesting resume/JD data from: {resume_jd_url}")
            logger.debug("🌐 Resume/JD response status: %s", resume_jd_response.status_code)
            logger.debug("Resume/JD response content: %s", resume_jd_response.text)
            ...
            if resume_jd_response.status_code == 200:
                ...
                session['email'] = resume_jd_data.get('email', '')
                session['candidate_name'] = resume_jd_data.get('candidate_name', '')
                username_external = resume_jd_data.get('candidate_name', 'Anonymous')
                organization_name_external = resume_jd_data.get('organization_name', 'Unknown')

                logger.debug("✅ Stored Resume & JD in session: %s", session)

                # Combine all data for template
                full_data = {**interview_data, **resume_jd_data}
                logger.debug("✅ Data sent to template: %s", full_data)

                return render_template("index.html", data=full_data)
            else:
                logger.warning("❌ Resume+JD request failed with status: %s", resume_jd_response.status_code)
                return render_template("error.html", message="❌ Unable to fetch resume and JD data."), 500

        elif interview_response.status_code == 403:
            logger.warning("✅ Interview already completed (403)")
            return render_template("error.html", message="✅ Interview already completed."), 403
        elif interview_response.status_code == 404:
            logger.warning("❌ Invalid or expired interview link (404)")
            return render_template("error.html", message="❌ Invalid or expired interview link."), 404
        elif interview_response.status_code == 410:
            logger.warning("❌ Interview link expired (410)")
            return render_template("error.html", message="❌ Interview link has expired."), 410
        else:
            logger.error("❌ Unexpected status code: %s", interview_response.status_code)
            return render_template("error.html", message="❌ Unexpected error. Please try again later."), 500

    except Exception as e:
        logger.error("❌ Exception in interview(): %s", str(e))
        return render_template("error.html", message="⚠ Server error while retrieving interview data."), 500    
