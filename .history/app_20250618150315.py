...
def interview(token):
    try:
        ...
        if response.status_code == 200:
            data = response.json()
            logger.debug(f"Data received from Django: {data}")

            match_id = data.get('id')
            if not match_id:
                logger.error("No match_id found in Django response")
            ...
            if resume_jd_response.status_code == 200:
                ...
                session.modified = True
                logger.debug(f"Rendering index.html with full_data: {full_data}")
                return render_template("index.html", data=full_data)
            else:
                logger.warning(f"Resume/JD API failed: {resume_jd_response.status_code}, {resume_jd_response.text}")
            ...
            logger.error(f"Unexpected status code: {response.status_code}, content: {response.text}")
