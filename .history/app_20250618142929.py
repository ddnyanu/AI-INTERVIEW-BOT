...
def interview(token):
    try:
        ...
        if response.status_code == 200:
            data = response.json()
            logger.debug("✅ Data received from Django: %s", data)
            ...
            if resume_jd_response.status_code == 200:
                ...
                logger.debug("Full data for template: %s", full_data)
                return render_template("index.html", data=full_data)
            else:
                logger.warning("Resume+JD not found or error: %s", resume_jd_response.text)
                return render_template("error.html", message="❌ Unable to fetch resume and JD."), 500
...
#                 session['job_title'] = resume_jd_data.get('job_title', '')
#                 session['email'] = resume_jd_data.get('email', '')
#                 session['candidate_name'] = resume_jd_data.get('candidate_name', 'Candidate')
#                 session.modified = True
#                 full_data = {**data, **resume_jd_data}
#                 return render_template("index.html", data=full_data)
#             else:
#                 logger.error(f"Resume/JD API failed: {resume_jd_response.text}")
#                 return render_template("error.html", message="Unable to fetch resume and JD."), 500
#         else:
#             logger.error(f"Django API failed: {response.status_code}, {response.text}")
#             return render_template("error.html", message=f"Error: {response.status_code}"), response.status_code
...
#         # Prepare conversation history with feedback
...
#         # Generate report content using Cohere
#         report_prompt = f"""
...
#         # Add pie chart HTML and JavaScript
#         chart_html = """
...
#         # Combine report content with chart
#         final_report_html = f"""
...
#     # Format conversation transcript with feedback
#     conversation_lines = []
#     i = 0
#     n = len(conv_history)
#     question_counter = 1
...
#     # Calculate ratings and performance
#     ratings = interview_data.get('ratings', [])
#     avg_rating = sum(ratings) / len(ratings) if ratings else 0
...
#     # Store answer, rating, and feedback
#     interview_data['answers'].append(answer)
#     interview_data['ratings'].append(rating)
#     interview_data['conversation_history'].append({
#         "speaker": "user",
